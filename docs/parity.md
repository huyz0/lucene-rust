...
| `codecs/lucene90/Lucene90DocValuesFormat`/`Lucene90DocValuesProducer` (`.dvm`/`.dvd` read, **NUMERIC fields only**) | `lucene-codecs/src/numeric_doc_values.rs` | ported (read-only), fixture-verified against a real IndexWriter with a plain-varying dense field, a GCD-compressed dense field, and a sparse field (via `IndexedDISI`); binary/sorted/sorted-set/sorted-numeric doc values and per-field doc-values skip indexes are out of scope (return `Error::UnsupportedFieldType`/`UnsupportedSkipIndex` rather than misdecode), as is the varying-bits-per-value block split (`Error::UnsupportedVaryingBpvBlocks`) — see the module doc |
| `util/packed/DirectReader` (bit-packed integer array read) | `lucene-codecs/src/numeric_doc_values.rs::direct_reader_get` | ported as **one generic bit-position formula**, not Java's thirteen width-specialized `DirectPackedReaderN` classes — those exist for JIT monomorphism, a concern this port doesn't have yet |
...
| `codecs/lucene90/Lucene90DocValuesProducer` binary/sorted/sorted-set/sorted-numeric doc values, `util/packed/DirectMonotonicReader`, terms-dict (LZ4) | — | not started — a separate future port (see `numeric_doc_values.rs` module doc for why it isn't just "skip past the bytes") |
