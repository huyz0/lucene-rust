...
| `codecs/lucene90/Lucene90DocValuesFormat`/`Lucene90DocValuesProducer` (`.dvm`/`.dvd` read, **NUMERIC, BINARY, and single-valued SORTED fields**) | `lucene-codecs/src/doc_values.rs` | ported (read-only), fixture-verified against a real IndexWriter: numeric plain-varying dense, numeric GCD-compressed dense, numeric sparse (`IndexedDISI`), binary fixed-length dense, binary variable-length dense (`DirectMonotonicReader`), binary variable-length sparse (`IndexedDISI` + `DirectMonotonicReader`), and a SORTED field with repeated values (ordinal reuse + terms dictionary); `SORTED_SET`/`SORTED_NUMERIC` (multi-valued) and per-field doc-values skip indexes are out of scope (return `Error::UnsupportedFieldType`/`UnsupportedSkipIndex` rather than misdecode), as is the varying-bits-per-value block split (`Error::UnsupportedVaryingBpvBlocks`) — see the module doc |
...
| `util/packed/DirectMonotonicReader` (monotonic sequence read, blocks of min/avg/bit-packed-delta) | `lucene-codecs/src/direct_monotonic.rs` | ported (read-only); used by BINARY doc values' variable-length address blocks and (structurally, values discarded) by the terms dictionary |
...
| `codecs/lucene90/Lucene90DocValuesProducer` `SORTED_SET`/`SORTED_NUMERIC` (multi-valued) doc values | — | not started — a separate future port (see `doc_values.rs` module doc) |
