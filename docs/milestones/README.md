...
| M2 | [OpenSearch read path](m2-opensearch-read-path.md) | A node answers `_search` from Rust over JNI/FFM | M–L | ✅ **delivered 2026-09-25** — an OpenSearch 3.8.0 node with the plugin serves the query phase from Rust per query, falling back to Lucene; OpenSearch's own REST YAML suites fail identically with and without it (501 tests), a 39-shape native-vs-Lucene matrix matches exactly, SIGKILL and force-merge release verified, JNI crossing 9.4 ns; shapes M1 never measured (boosts, `must_not`, mixed booleans) are correct natively but 3–5× slower, so they are routed to Lucene |
...
| M5 | [Engine integration](m5-engine-integration.md) | A shard fully served by Rust — indexing and search | XL | not started — unblocked: M2 and M4 are both delivered |
