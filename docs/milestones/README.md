...
| M3 | [Write path proven](m3-write-path-proven.md) | Real Lucene reads a full Rust-written index | L | ✅ **delivered** — 120k-doc, 5-segment Rust-written index: every term's postings, 59 queries (scores within 5e-10) and `CheckIndex` agree with Lucene; term-dictionary writer byte-identical to Java and 1.3×–1.9× its speed |
