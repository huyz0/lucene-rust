...
| M3 | [Write path proven](m3-write-path-proven.md) | Real Lucene reads a full Rust-written index | L | ✅ **delivered** — 120k-doc, 5-segment Rust-written index: every term's postings, 62 queries (scores within 1.2e-7) and `CheckIndex` agree with Lucene; term-dictionary writer byte-identical to Java and 1.3×–1.9× its speed |
