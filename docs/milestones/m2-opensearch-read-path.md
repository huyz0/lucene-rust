...
| REST suite | OpenSearch's own YAML suites (`search`, `search.highlight`, `search.inner_hits`, `msearch`, `scroll`, `count`, `explain`, `suggest`, `get`, `index`, `delete`, `bulk`, `update`, `mget`, `exists` — 501 tests) run against the plugin node **and** a stock node from the same image: **identical failure sets** (the same 4 `_source`-filtering warning-header tests fail on both), 123 query phases ran native, 0 native errors (`verify-opensearch.sh --yaml`) |
| fallback is correct | `opensearch-plugin/e2e/verify_opensearch.py`: 43 request shapes × 2 indices (1 and 3 shards), in both routing modes, each run with the plugin off (the reference) and on; hits, scores (1e-5), totals and max score must match, and the plugin's counters must show the expected route and reason. 1,061 checks, 0 failures |
