...
2. Read `.agents/skills/code-review/SKILL.md` (the rubric) and
   ...
   `port-workflow`, `differential-testing`, `rust-performance`, and
   `architecture` skills.
...
- **Port → benchmark → optimise** — see `port-workflow`. A newly ported area
  must first be the closest-to-Java port (same algorithm, structures and
  constants; Rust-forced changes named in the module doc) — flag an
  unmeasured redesign as GATING. A diff that finishes an area must carry a
  `scripts/bench-micro.sh`/`bench-compare.sh` ratio against Lucene; below 1.0
  needs a written-up cause. Optimisations should be on Rust's terms
  (`rust-performance`) and cite the measurement behind them.
