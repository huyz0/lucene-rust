...
state (this facade does not call `merge_policy::find_merges` or
`merge::merge_stored_only_segments` itself -- see "explicitly deferred"
below). Segment/commit ids are generated by hashing a monotonic counter with
...
automatic merge triggering -- `commit()` never calls `merge_policy::
find_merges`; that automatic-trigger wiring is real, separate future work
(this port's merge-policy decision function and merge executor both already
exist standalone, but nothing today calls the first and feeds its output to
the second automatically), so `IndexWriter::segment_infos()` is exposed
precisely so a caller can still drive that pair by hand in the meantime. No
