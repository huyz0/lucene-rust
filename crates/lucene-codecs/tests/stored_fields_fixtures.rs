//! Differential test against real `.fdt`/`.fdx`/`.fdm` files written by an
//! actual IndexWriter: 6 documents, each with one field of every supported
...
impl Manifest {
    fn load() -> Self {
        let text = std::fs::read_to_string(format!("{}manifest.properties", dir()))
            .expect("run fixtures generator first (GenStoredFields)");
        let kv = text
            .lines()
...
/// Parses one `name:type:value` entry from the manifest's `;`-joined field list.
...
#[test]
fn parses_real_stored_fields_and_matches_lucene_values() {
    ...
    for doc_id in 0..max_doc {
        ...
        // Fields aren't necessarily in manifest order, but there's exactly
        // one of each name here, so match by position after both are
        // collected -- this port doesn't expose field *names*, only numbers,
        // so cross-check by value set instead.
...
fn wrong_segment_id_rejected() {
    ...
    let fdm = std::fs::read(format!("{}{}.raw", dir(), manifest.get("fdm_file_name"))).unwrap();
    let wrong_id = [9u8; 16];
    assert!(stored_fields::open(&fdt, &fdx, &fdm, &wrong_id, "").is_err());
}
