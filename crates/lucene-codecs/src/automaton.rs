...
#[derive(Debug, Clone)]
pub struct ByteDfa {
    /// `table[state * 256 + byte]`, [`DEAD`] where no live state follows.
    table: Vec<u32>,
    accept: Vec<bool>,
    start: u32,
...
impl ByteDfa {
    ...
    pub fn step(&self, state: u32, byte: u8) -> u32 {
        ...
        // and the table has exactly 256 entries per state.
        self.table[(state as usize) << 8 | byte as usize]
    }
...
impl Nfa {
    ...
    pub fn determinize(&self, start: u32, accept: u32) -> Option<ByteDfa> {
        ...
        self.closure(&mut first, &mut stack, &mut seen);
        ...
        while next < sets.len() {
            edges.clear();
            for &s in &sets[next] {
                edges.extend_from_slice(&self.ranges[s as usize]);
            }
            ...
            for w in cuts.windows(2) {
                ...
                self.closure(&mut target, &mut stack, &mut seen);
